import { filterDay, getDaytimeData, summarize, getTideExtremes } from './openmeteo.js';
...
export function formatDayForecast(beach, data, daysOffset = 0) {
  ...
  // Maré
  const tideExtremes = getTideExtremes(todayData);
  if (tideExtremes.length) {
    const tideParts = tideExtremes.map(e => {
      const emoji = e.type === 'alta' ? '🔼' : '🔽';
      return `${emoji} ${e.type} ${e.time} (${e.height}m)`;
    });
    msg += `\n🌊 *Maré:* ${tideParts.join(' · ')}\n`;
    msg += `_⚠️ Previsão aproximada (modelo oceânico 8km)_\n`;
  }
