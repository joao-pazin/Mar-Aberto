...
export async function sendScheduledAlert(subscriber, allBeachData, type = 'morning') {
  ...
  // Rodapé
  const { isPremium } = await import('./billing.js');
  const upgradeNote = !isPremium(subscriber)
    ? `\n/upgrade — assinar Premium (17 praias por R$7,90/mês)` : '';

  await sendMessage(chatId,
    `━━━━━━━━━━━━━━━━\n/previsao — ver previsão de hoje\n/semana — ver previsão da semana\n/praias — alterar praias monitoradas${upgradeNote}`,
    { parse_mode: 'Markdown' }
  );
}
...
/**
 * Gera um parágrafo fluido explicando os fatores da classificação.
 */
export function getRatingDescription(summary, beach = null) {
  const waveH   = summary.waveHeight?.avg || 0;
  const waveMax = summary.waveHeight?.max || 0;
  ...
  // ── Onda ─────────────────────────────────────────────────
  if (waveH >= 0.8 && waveH <= 2.5) {
    const adj = waveH >= 1.8 ? 'com bom tamanho' : waveH >= 1.2 ? 'de tamanho ideal' : 'pequenas mas surfáveis';
    parts.push(`Ondas ${adj} (${fmt(waveH)}m)`);
  } else if (waveH > 2.5 && waveH <= 4) {
    parts.push(`Mar grande (${fmt(waveH)}m, pico até ${fmt(waveMax)}m) — recomendado para surfistas experientes`);
  } else if (waveH > 4) {
    parts.push(`Mar muito grande (${fmt(waveH)}m) — perigoso para a maioria`);
  } else {
    parts.push(`Ondas fracas (${fmt(waveH)}m)`);
  }

  // ── Período ──────────────────────────────────────────────
  if (period >= 14)      parts.push(`período longo de ${fmt(period)}s traz potência e organização`);
  else if (period >= 12) parts.push(`período de ${fmt(period)}s garante ondas organizadas`);
  else if (period >= 9)  parts.push(`período moderado de ${fmt(period)}s`);
  else                   parts.push(`período curto (${fmt(period)}s) — marolinha bagunçada`);

  // ── Alinhamento do swell ──────────────────────────────────
  if (swellDir !== null && beach?.facing) {
    ...
    if (facingDeg !== null) {
      ...
      if (diff <= 22.5)      parts.push(`swell ${swellLabel} chegando direto na praia`);
      ...
      else                   parts.push(`swell ${swellLabel} desfavorável para esta praia`);
    }
  }

  // ── Vento ─────────────────────────────────────────────────
  if (windDir !== null && beach?.facing) {
    const facingDeg = dirToDegs(beach.facing);
    if (facingDeg !== null) {
      let diff = Math.abs(windDir - facingDeg);
      if (diff > 180) diff = 360 - diff;
      const isOffshore = diff >= 135;
      const isOnshore  = diff <= 45;
      const windLabel  = degreesToDir(windDir);
      if (isOffshore) {
        if (wind < 10)       parts.push(`vento offshore leve (${fmt(wind)} km/h) — condições perfeitas`);
        else if (wind < 20)  parts.push(`vento offshore moderado (${fmt(wind)} km/h) favorece as ondas`);
        else                 parts.push(`vento offshore forte (${fmt(wind)} km/h)`);
      } else if (isOnshore) {
        if (wind < 10)       parts.push(`vento onshore fraco (${fmt(wind)} km/h)`);
        else if (wind < 20)  parts.push(`vento onshore (${fmt(wind)} km/h) atrapalha um pouco`);
        else                 parts.push(`vento onshore forte (${fmt(wind)} km/h) prejudica as condições`);
      } else {
        if (wind < 15)       parts.push(`vento lateral fraco (${fmt(wind)} km/h)`);
        else                 parts.push(`vento lateral de ${fmt(wind)} km/h`);
      }
    }
  } else {
    if (wind < 10)       parts.push(`vento fraco (${fmt(wind)} km/h)`);
    else if (wind < 20)  parts.push(`vento moderado (${fmt(wind)} km/h)`);
    else if (wind < 30)  parts.push(`vento forte (${fmt(wind)} km/h) atrapalha`);
    else                 parts.push(`vento muito forte (${fmt(wind)} km/h) compromete as condições`);
  }
