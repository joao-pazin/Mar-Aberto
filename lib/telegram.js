...
export async function sendScheduledAlert(subscriber, allBeachData, type = 'morning') {
  ...
  // Previsão do dia (hoje ou amanhã) — sempre
  ...
  // Previsão da semana — SÓ no domingo à noite (prévia da semana que começa)
  const isSundayEvening = !isMorning &&
    new Date().toLocaleString('en-US', { timeZone: 'America/Sao_Paulo', weekday: 'short' }) === 'Sun';
  ...
  // Rodapé
  await sendMessage(chatId,
    `━━━━━━━━━━━━━━━━\nPara ver a previsão da semana, digite /semana\n/praias — alterar praias monitoradas`,
    { parse_mode: 'Markdown' }
  );
}
...
export function formatDayForecast(beach, data, daysOffset = 0) {
  ...
  const swellDirDisplay = validNum(detail.swellDirection) ?? validNum(detail.waveDirection);

  msg += `🌐 *Swell*\n`;
  msg += `   Altura: ${fmt(detail.swellHeight.avg)}m\n`;
  msg += `   Período: ${fmt(detail.swellPeriod.avg)}s\n`;
  msg += `   Direção: ${degreesToDir(swellDirDisplay)}`;

  if (swellDirDisplay !== null && beach?.facing) {
    const facingDeg = dirToDegs(beach.facing);
    if (facingDeg !== null) {
      let diff = Math.abs(swellDirDisplay - facingDeg);
...
function rateHour(h, beach) {
  return computeRating({
    ...
    wind: h.windSpeed || 0,
    windDir: validNum(h.windDirection),
    rain: h.precipitation || 0,
    swellDir: validNum(h.swellDirection) ?? validNum(h.waveDirection),
...
export function getRating(summary, beach = null) {
  return computeRating({
    ...
    windDir: validNum(summary.windDirection),
    rain: summary.precipitation || 0,
    swellDir: validNum(summary.swellDirection) ?? validNum(summary.waveDirection),
...
function computeRating({ waveH, period, wind, windDir, rain, swellDir, swellPeriod }, beach) {
  ...
  // Quando scoreBase <= 0 (mar ruim/muito ruim), multiplicadores positivos
  // não amplificam a nota — seria absurdo vento bom deixar mar péssimo "mais péssimo".
  // Cap em 1.0 para multiplicadores nesse caso.
  const effectivePeriodMult = scoreBase <= 0 ? Math.min(periodMult, 1.0) : periodMult;
  const effectiveWindMult   = scoreBase <= 0 ? Math.min(windMult, 1.0)   : windMult;

  const score = (scoreBase * effectivePeriodMult * effectiveWindMult) - chuvaPenalty;

  // ─── DEBUG TEMPORÁRIO ────────────────────────────────────────────────────
  console.log('[RATING]', JSON.stringify({
    waveH: waveH.toFixed(2), period: period.toFixed(1),
    wind: wind.toFixed(1), windDir, swellDir,
    swellPeriod: swellPeriod.toFixed(1),
    heightScore: heightScore.toFixed(2),
    alignScore,
    scoreBase: scoreBase.toFixed(2),
    periodMult: periodMult.toFixed(2),
    windMult: windMult.toFixed(2),
    effectivePeriodMult: effectivePeriodMult.toFixed(2),
    effectiveWindMult: effectiveWindMult.toFixed(2),
...
export function getRatingDescription(summary, beach = null) {
  ...
  const swellDir = validNum(summary.swellDirection) ?? validNum(summary.waveDirection);
  ...
  if (w.label === 'sem_vento') {
    ...
    if (w.intensity === 'leve')     parts.push(`vento offshore leve (${fmt(wind)} km/h)`);
    else if (w.intensity === 'moderado') parts.push(`vento offshore (${fmt(wind)} km/h)`);
...
/**
 * Retorna o valor se for um número finito válido, senão null.
 * Trata null, undefined, NaN e Infinity de forma uniforme.
 */
...
function dirToDegs(dir) {
  ...
  const map = {
    ...
    // Inglês (E, W, SW, NW) — aceita as duas convenções
    E: 90, W: 270, SW: 225, NW: 315,
  };
