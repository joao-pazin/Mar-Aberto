...
function swellAlignmentScore(swellDirectionDeg, beachFacing) {
  ...
  if (diff <= 22.5)  return 3;  // direto
  if (diff <= 45)    return 2;  // levemente oblíquo
  if (diff <= 67.5)  return 1;  // oblíquo
  if (diff <= 90)    return 0;  // paralelo
  return -1;                    // de costas
}
...
export function getRating(summary, beach = null) {
  ...
  score += swellAlignmentScore(swellDir, beach?.facing);
