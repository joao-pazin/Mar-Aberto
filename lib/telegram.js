...
export function formatDayForecast(beach, data, daysOffset = 0) {
  ...
  console.log(`[DEBUG ${beach.id}]`, JSON.stringify({
    totalSamples: sourceData.length,
    firstSample: sourceData[0],
    lastSample: sourceData[sourceData.length - 1],
    summary: {
      waveHeight: summary?.waveHeight,
      wavePeriod: summary?.wavePeriod,
      swellHeight: summary?.swellHeight,
      swellPeriod: summary?.swellPeriod,
      swellDirection: summary?.swellDirection,
      waveDirection: summary?.waveDirection,
      windSpeed: summary?.windSpeed,
      windDirection: summary?.windDirection,
    },
    rating: { label: rating.label, score: rating.score?.toFixed(2) },
    hasWindow: !!window,
  }, null, 2));
  // ────────────────────────────────────────────────────────────────────────
