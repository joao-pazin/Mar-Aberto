...
export function summarize(hourlyData) {
  ...
  // Helper para validar se um valor é um número utilizável
  const isValidNum = v => v !== null && v !== undefined && Number.isFinite(v);

  const avg = key => {
    const vals = hourlyData.map(d => d[key]).filter(isValidNum);
  ...
  // Direção mais comum (moda) — agrupa em octantes de 45°
  const modeDirection = key => {
    const vals = hourlyData.map(d => d[key]).filter(isValidNum);
    if (!vals.length) return null;
    const rounded = vals.map(v => Math.round(v / 45) * 45 % 360); // normaliza 360 → 0
    const freq = {};
    rounded.forEach(v => { freq[v] = (freq[v] || 0) + 1; });
    const entry = Object.entries(freq).sort((a, b) => b[1] - a[1])[0];
    const result = parseInt(entry[0]);
